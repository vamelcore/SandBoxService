...
session_start();
if ((isset($_SESSION['user_id'])) && ($_SESSION['password_secret_id_string'] == md5($_SERVER['REMOTE_ADDR'].$_SESSION['user_id'].session_id().'6891')) && ($_SESSION['vot_priv_voz'] == 1)) {
...
<tr>
<td style="border-bottom:1px solid #c6d5e1;"><?php if ($_SESSION['tabl_store_show'][1] <> '1') {header("Location: index.php"); die();} include ("includers/header_mag.php");?></td>
</tr>
	...
	<table cellpadding="0" cellspacing="0" border="0" id="table" class="sortable">
		<thead>
			<tr>
				...
				<th><h3>Обмен на</h3></th>
				<th><h3>Дальнейшее движение</h3></th>
				<th><h3>Примечания</h3></th>
...
printf("<td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>",$myrow['data'],$myrow['t_a'],$myrow['kategoria'],$myrow['naimenovanie'],$myrow['esn'],$myrow['kolichestvo'],$myrow['data_pokupki_vozvr_ob'],$myrow['summa_nal'],$myrow['summa_bez_nal'],$myrow['prichina_vozvrata'],$myrow['per_14_dney'],$myrow['obmen_na'],$myrow['daln_dvijenie_vozvr_tov'],$myrow['primechanie'],$myrow['kto_pvinyal']);
}
